<template>
  <div id="mapDiv" style="width: 100%; height: 90vh;" />
</template>
  ...
  methods: {
    initMap() {
      ...
      getConfigKey('base_location').then(res => {
        const val = res.data.configValue
      ...
      listHouse().then(response => {
        ...
        this.houseList.forEach(house => {
          if (house.longitude.length > 0) {
            const lnglat = new T.LngLat(house.longitude, house.latitude)
            ...
            const content = '户主: ' + house.holder + '<br/>' +
