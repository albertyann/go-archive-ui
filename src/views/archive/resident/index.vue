  ...
  <BasicLayout>
    <template #wrapper>
      <el-card class="box-card">
            ...
            <el-row>
              ...
              <el-col :span="12">
                <el-form-item label="证件有效期">
                  <el-input v-model="form.idCardStart" :style="{width: '120px'}" />
                  <span> 至 </span>
                  <el-input v-model="form.idCardEnd" :style="{width: '130px'}" />
                </el-form-item>
              </el-col>
            </el-row>
              ...
              <el-col :span="8">
                <el-form-item label="户籍类型" prop="hukouType">
                  <el-select v-model="form.hukouType" clearable :style="{width: '100%'}">
                    <el-option
                      v-for="(item, index) in hukouTypeOptions"
                      :key="index"
                      :label="item.label"
                      :value="item.value"
                      :disabled="item.disabled"
              ...
              <el-col :span="8">
                <el-form-item label="户籍分组" prop="hukouGroup">
                  <el-select v-model="form.hukouGroup" placeholder="请选择户籍分组" clearable :style="{width: '100%'}">
                    <el-option
                      v-for="(item, index) in hukouGroupOptions"
                      :key="index"
                      :label="item.label"
                      :value="item.value"
                      :disabled="item.disabled"
              ...
              <el-col :span="12">
                <el-form-item label="是否户主" prop="isHolder">
                  <el-select v-model="form.isHolder" placeholder="请选择是否户主" clearable :style="{width: '100%'}">
                    <el-option
                      v-for="(item, index) in isHolderOptions"
                      :key="index"
                      :label="item.label"
                      :value="item.value"
                    />
              ...
              <el-col :span="12">
                <el-form-item label="户主关系" prop="holderRelation">
                  <el-select v-model="form.holderRelation" placeholder="请选择户主关系" clearable :style="{width: '100%'}">
                    <el-option
                      v-for="(item, index) in holderRelationOptions"
                      :key="index"
                      :label="item.label"
                      :value="item.value"
                      :disabled="item.disabled"
              ...
              <div v-if="form.isHolder == 1">
                <el-col :span="24">
                  <el-form-item label="户籍成员" prop="status">
                    <el-button size="mini" @click="openDialog = true">添 加</el-button>
                  </el-form-item>
                </el-col>
                <el-col :span="24">
                  <el-form-item>
                    <el-table :data="familyMember">
                      ...
                      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
                        <template slot-scope="scope">
                          <el-button type="text" size="mini" @click="handleMemberDelete(scope.row)">删除</el-button>
                        </template>
                      </el-table-column>
                    </el-table>
                  </el-form-item>
              ...
              <el-col :span="12">
                <el-form-item label="婚姻状况" prop="married">
                  <el-select v-model="form.married" placeholder="请选择婚姻状况" clearable :style="{width: '100%'}">
                    <el-option
                      v-for="(item, index) in marriedOptions"
                      :key="index"
                      :label="item.label"
                      :value="item.value"
                      :disabled="item.disabled"
              ...
              <el-col :span="12">
                <el-form-item label="学历" prop="diploma">
                  <el-select v-model="form.diploma" placeholder="请选择学历" clearable :style="{width: '100%'}">
                    <el-option
                      v-for="(item, index) in diplomaOptions"
                      :key="index"
                      :label="item.label"
                      :value="item.value"
                      :disabled="item.disabled"
              ...
              <el-col :span="12">
                <el-form-item label="残疾" prop="disability">
                  <el-select v-model="form.disability" placeholder="请选择残疾" clearable :style="{width: '100%'}">
                    <el-option
                      v-for="(item, index) in disabilityOptions"
                      :key="index"
                      :label="item.label"
                      :value="item.value"
                      :disabled="item.disabled"
              ...
              <el-col :span="12">
                <el-form-item label="残障等级" prop="disabilityLevel">
                  <el-select v-model="form.disabilityLevel" placeholder="请选择残障等级" clearable :style="{width: '100%'}">
                    <el-option
                      v-for="(item, index) in disabilityLevelOptions"
                      :key="index"
                      :label="item.label"
                      :value="item.value"
                      :disabled="item.disabled"
              ...
              <el-col :span="12">
                <el-form-item label="是否低保" prop="guarantee">
                  <el-select v-model="form.guarantee" placeholder="请选择是否低保" clearable :style="{width: '100%'}">
                    <el-option
                      v-for="(item, index) in guaranteeOptions"
                      :key="index"
                      :label="item.label"
                      :value="item.value"
                      :disabled="item.disabled"
              ...
              <el-col :span="12">
                <el-form-item label="是否拆迁" prop="demolition">
                  <el-select v-model="form.demolition" placeholder="请选择是否拆迁" clearable :style="{width: '100%'}">
                    <el-option
                      v-for="(item, index) in demolitionOptions"
                      :key="index"
                      :label="item.label"
                      :value="item.value"
                      :disabled="item.disabled"
              ...
              <el-from-item>
                <el-col :span="24">
                  <el-form-item label="车辆信息" prop="status">
                    <el-button size="mini" @click="handleAddCar">添 加</el-button>
                  </el-form-item>
                </el-col>
                <el-col :span="24">
                  <el-form-item label="">
                    <el-table :data="carList">
                      <el-table-column label="车牌" align="center">
                        <template slot-scope="scope">
                          <el-input v-if="scope.row.id==0" v-model="scope.row.carNo" placeholder="车牌" />
                          <span v-if="scope.row.id>0">{{ scope.row.carNo }}</span>
                        </template>
                      </el-table-column>
                      <el-table-column label="车主" align="center">
                        <template slot-scope="scope">
                          <el-input v-if="scope.row.id==0" v-model="scope.row.ownerName" placeholder="车主" />
                          <span v-if="scope.row.id>0">{{ scope.row.ownerName }}</span>
                        </template>
                      </el-table-column>
                      <el-table-column label="联系电话" align="center">
                        <template slot-scope="scope">
                          <el-input v-if="scope.row.id==0" v-model="scope.row.ownerPhone" placeholder="联系电话" />
        ...
        <DetailView :open-view="openView" @handleClose="openView = $event" />
      </el-card>
    </template>
  </BasicLayout>
</template>
